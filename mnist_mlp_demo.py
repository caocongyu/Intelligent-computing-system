...
class MNIST_MLP(object):
    def __init__(self):
        ...
        self.input_quant_params = [] # 输入数据的量化参数
        self.filter_quant_params = [] # 模型参数的量化参数
    ...
    def build_model(self, batch_size=100, input_size=784, 
        ...
        # 创建 ReLu 激活函数层
        self.net.createReLuLayer('relu1') #_____________________
        self.net.createMlpLayer('fc2', hidden2, self.input_quant_params[0])#____________
        self.net.createReLuLayer('relu2')#_______________
        self.net.createMlpLayer('fc3', out_classes, self.input_quant_params[0])#_____________
        # 创建 Softmax 损失层
        self.net.createSoftmaxLayer('softmax', axis=1) #__________________
    ...
    def load_data(self, data_path, label_path):
        ...
        test_images = self.load_mnist(os.path.join(MNIST_DIR, TEST_DATA), True) # __________________
        test_labels = self.load_mnist(os.path.join(MNIST_DIR, TEST_LABEL), False) #__________________
    ...
    def load_model(self, param_dir):  # 加载参数
        ...
        self.net.loadParams(2, weigh2, bias2, self.filter_quant_params[0]) #为指定的层加载参数__________________

        weigh3 = np.transpose(params['w3'], [1, 0]).flatten().astype(np.float64)
        bias3 = params['b3'].flatten().astype(np.float64)
        self.net.loadParams(4, weigh3, bias3, self.filter_quant_params[0]) #__________________
